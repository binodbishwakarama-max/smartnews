...
import { Article, useBookmarks } from '../contexts/BookmarkContext';
...
export default function BookmarkButton({ article, className = "", showText = false }: BookmarkButtonProps) {
    ...
    const isActive = isBookmarked(article.id);
    ...
    return (
        ...
        >
            <div
                className={`relative p-2 rounded-full transition-colors duration-300 ${isActive
                    ? 'bg-accent/10 text-accent'
                    : 'text-secondary hover:bg-black/5 dark:hover:bg-white/10 hover:text-black dark:hover:text-white'
                    }`}
            >
                <Bookmark
                    className={`w-5 h-5 transition-all duration-300 ${isActive ? 'fill-accent scale-110' : 'scale-100'
                        }`}
                />
            </div>
            ...
            {showText && (
                <span
                    className={`text-[10px] font-bold uppercase tracking-widest transition-colors duration-300 ${isActive
                        ? 'text-accent'
